...
select * from docente where `CO_ENTIDADE` = CO_ENTIDADE and `NU_IDADE` <= 45;
...
select escola.`NO_ENTIDADE`, turma.`ID_TURMA` from turma
join escola on escola.`CO_ENTIDADE` = turma.`CO_ENTIDADE`
where turma.`TP_ETAPA_ENSINO` in (39,40,64);

-- Selecione a quantidade total de computadores de alunos e administrativos na cidade;
select sum(`NU_COMP_ADMINISTRATIVO`) + sum(`NU_COMP_ALUNO`) as total_computadores FROM escola;
