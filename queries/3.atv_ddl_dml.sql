...
alter table usuario modify data_cadastro datetime default current_timestamp;

-- Adicione o campo senha na tabela Usuario, com valor default de sha(‘senha’) e de até 40 caracteres;
-- alter table usuario add senha varchar(40) default SHA('senha') not null;
alter table usuario add senha varchar(40) not null;
create trigger set_default_password before insert on usuario for each row
set new.senha = SHA('senha');
...
values ('admin','admin@unesp.br','admin','1999-01-01');

insert into bookmark (id_usuario, id_entidade)
values (1, 35004193);
